		...
		if (data ~= nil) then
			...
			elseif (topic == config.ENDPOINT.."/"..owner and jobj.applianceserial == sn) then
				...
				elseif (jobj.msg == "cmdid" and jobj.cmdid ~= -1) then
					...
					print(toArduino)
					tempMsg = ""
				elseif(jobj.msg=="boardStatus" and not initStatus)then
					--toArduino = "{\"msg\":\"setBoardUpdateInterval\",\"cmdid\":"..jobj.cmdid..",\"updateinterval\":"..jobj.upint.."}"
					--toArduino = cjson.encode(jobj)
